...
<?php 
	...
	// Seulement si le point focal de courrier arrivé s'est authentifié  
 	...
 	// if (($_SESSION['profile_special']!= "chefdev" && $_SESSION["id"] != '55') || ($_SESSION['profile_special']!= "coordo" && $_SESSION["id"] != '42')) {
			 // $_SESSION["errorType"] = "danger";
        // $_SESSION["errorMsg"] = "Vous n'êtes pas autorisé à accéder à ce module.";
		// redirect("logout.php");
		// exit;	
			
	// }
	if($_SESSION['mod_modulename']!="Administration"){
	...
	if($_SESSION["rr_view"] =="no" || $_SESSION["rr_view"] ==""){
			// Ce n'est pas le veritable point focal de ce module
		$_SESSION["errorType"] = "danger";
        $_SESSION["errorMsg"] = "Vous n'avez pas les droit d'accès à ce module, veuillez contacter l'administrateur";
		redirect("../logout.php");
		exit;		
	}			
...
<!--DEBUT :: LES META -->
    <meta charset="utf-8">
    <title>SIE:: Administration Coordination</title>
    	<meta charset="utf-8">
...
<!--DEBUT :: BOOTSTRAP CSS et SCRIPT-->  
 ...
 <!-- jQuery 2.1.4 -->
  ...
  <tr>
  	<td style="text-align:left"><span style="font-size: 13px; font-weight: bold; text-transform: uppercase;">GESTION <?php echo $_SESSION['mod_modulename']; ?></span><span> -- <?php echo $_SESSION['nomprenom']; ?></span> | 
	<a href="#" id="<?php echo $_SESSION['id']; ?>" class="btn btn-danger btnedit_p" role="button" ><i class="fa fa-edit"></i>
		Modifier votre mot de passe</a></td>
...
<!--Espace d'affichage info dynamique telle que ERREUR! AVERTISSEMENT! ...etc -->
			     ...
			     <div class="box">
                     <p>
                        <button type="submit" class="btn btn-succes " id="btnassignation" name="btnassignation"><i class="fa fa-plus"></i> Assignation de modules </button>
                         <a class="btn btn-primary" href="users.php" role="button">Gestion des utilisateurs</a>
                        ...
                        <a class="btn btn-primary" href="ministere/structures_admin.php" role="button">Gestion des structures</a>
                         ...
                         <a class="btn btn-primary" href="ministere/banners_admin.php" role="button">Gestion des bannieres</a>
                     <p> 
                     
                    </p>     
				<?php
					if(isset($_SESSION['id']) && $_SESSION['id'] == 55){
						echo '<a class="btn btn-danger" href="logsession.php" role="button">Log Session</a>';				
					}
				?>
				...
				<table id="table_data" class="datatable" >
                  <thead>
                      <tr class="tableheader">
                        ...
                        <th>Droit de vue</th>
                        <th>Droit de création</th>
                        <th>Droit de <br/>modification</th>
						<th>Droit de <br/>suppression</th>
						<th>Droit activé</th>
						...
						<?php include('modal/modal_edit_assignation_droit.php'); ?>
		...
		<script src="web_admin.js"></script> 
